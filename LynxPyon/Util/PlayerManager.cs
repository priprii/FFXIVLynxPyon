...
namespace LynxPyon {
    public unsafe class PlayerManager : IDisposable {
        ...
        public PlayerManager() {
            UpdateParty updateParty = new(LynxPyon.SigScanner);
        ...
        public void UpdatePlayers(ref List<Player> players) {
            List<Player> nearbyPlayers = new List<Player>();

            for(int i = 0; i < 200; i += 2) {
                var obj = LynxPyon.Objects[i];
                if(obj is not PlayerCharacter player) {
                    continue;
                }
                nearbyPlayers.Add(new Player((int)player.ObjectId, player.Name.TextValue));
            }

            foreach(Player player in players) {
                if(nearbyPlayers.Find(x => x.ID == player.ID) == null) {
                    player.Name = "";
                }
            }
